...
import { deleteObject, getObject, putObject } from '@/lib/object-store'
 ...
 * WHERE THE BYTES GO
 * ------------------
 * lib/object-store, which is local disk in development and on a host with a
 * persistent volume, and a PRIVATE Vercel Blob store on a serverless host
 * whose filesystem is ephemeral. Nothing in this file knows which; it only
 * knows keys. A private store matters here: a public one hands out URLs that
 * work for anyone holding them, and these are identity documents.
 */
...
export async function storeUpload(file: File, kind: UploadKind): Promise<StoreResult> {
  ...
  await putObject('uploads', key, buf, actual)
...
export async function readUpload(id: string): Promise<StoredFile | null> {
  ...
  // storageKey is generated by this module, never by a caller. The object
  // store still refuses any key that would escape its root, which closes off
  // path traversal for good if that ever stops being true.
  const bytes = await getObject('uploads', row.storageKey)
  return bytes ? { bytes, contentType: row.contentType, filename: row.filename } : null
}
...
export async function removeUpload(id: string): Promise<void> {
  ...
  await deleteObject('uploads', row.storageKey)
